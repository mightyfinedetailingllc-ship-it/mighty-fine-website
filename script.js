...
const db=window.supabase?.createClient(SB_URL,SB_KEY,{auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false}}),TRACK_EVENTS=new Set(["page_view","call_click","text_click","quote_start","estimate_submit"]);
function track(event_name){if(!db||!TRACK_EVENTS.has(event_name))return;db.from("mf_web_events").insert({business_id:BUSINESS_ID,event_name,page_path:location.pathname.slice(0,255)}).then(()=>{}).catch(()=>{})}
track("page_view");document.querySelectorAll('a[href^="tel:"]').forEach(a=>a.addEventListener("click",()=>track("call_click")));document.querySelectorAll('a[href^="sms:"]').forEach(a=>a.addEventListener("click",()=>track("text_click")));
...
form.querySelectorAll('[data-step="1"] [data-choice]').forEach(b=>b.onclick=()=>{startQuote();vehicle=b.dataset.choice;step(service?3:2)});
...
document.querySelectorAll(".service button").forEach(b=>b.onclick=()=>{service=b.dataset.service;document.getElementById("book").scrollIntoView({behavior:"smooth",block:"start"});step(1)});
form.onsubmit=async e=>{
 e.preventDefault();const n=document.getElementById("name").value.trim(),p=document.getElementById("phone").value.trim(),c=document.getElementById("city").value.trim(),submit=form.querySelector('button[type="submit"]'),digits=p.replace(/\D/g,'');
 if(!n||!p||!c||!vehicle||!service)return;if(n.length>120||c.length>120||p.length>32||digits.length<7){document.getElementById("summary").textContent="Please check your name, phone number and city, then try again.";step(4);return}
 submit.disabled=true;submit.textContent="Sending Request…";let saved=false;
 ...
 if(saved)track("estimate_submit");document.getElementById("summary").textContent=saved?n+", your estimate request was sent to Mighty Fine. You can also text us below if you'd like.":n+", your request is ready to text to Mighty Fine.";
 const msg=encodeURIComponent("Hi! I'm "+n+". I'd like a quote for a "+service+" on my "+vehicle+". I'm located in "+c+".");document.getElementById("textRequest").href="sms:+13306415559?&body="+msg;step(4);submit.disabled=false;submit.textContent="Send My Estimate Request →";
};
